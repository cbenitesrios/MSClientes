...
public class MSClientServiceImpl implements MSClientService {
	...
	public  Mono<Client> createclient(CreateClientRequest createclientrequest) {  
		Mono<Client> cliente = clientrepo.findByClientcode(createclientrequest
				.getClientrequest()
				.getClientcode()); 
		
		
		return cliente.defaultIfEmpty( new Client(null,
	...
	public Mono<Client> findclient(String clientcode) { 
		 return clientrepo.findByClientcode(clientcode).switchIfEmpty( Mono.error(new Exception("No se encontro entidad")));	
	}
	
	@Override
	public Flux<Client> findallclient() {
		return clientrepo.findAll().switchIfEmpty(Mono.error(new Exception("No se encontrontraron resultados")));
	}
